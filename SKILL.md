...
1. 用 SubAgent 运行 `python ./scripts/check_status/check_status.py <input_bin>`（在本 Skill 目录下执行）。
  - SubAgent 直接返回脚本输出，不需要额外说明，返回的输出即为当前运行状态：
...
2. 如果检测结果是 `[First Run]`，则需要运行 Layer1 Source Func Skill
  ...
  - **重要**：运行后检查输出中是否有 `[PENDING]` 标记。如有，说明存在待 SubAgent 判断的外部函数，需要：
    - 读取 pending JSON 文件
    - 用 SubAgent 逐个判断每个函数是否为 Source 函数：
      - 判定为 Source：必须在结果中附带 `reason` 字段
        - 若是标准库函数，引用所属标准库/API 作为理由（如 fopen → C标准库 stdio.h）
        - 若非标准库函数，给出强证据说明为何能接收外部控制数据
      - 判定为非 Source：不需要理由
    - 调用 `apply_subagent_results()` 更新缓存，格式如下：
      `apply_subagent_results({"func1": {"is_source": True, "reason": "C标准库stdio.h，从文件读取外部数据"}, "func2": {"is_source": False}})`
    - 重新运行 Layer1 Source Func Skill 获取完整结果
